...
from tensorflow.keras.layers import Input, Conv3D, MaxPooling3D, Flatten, Dense, Dropout, BatchNormalization, Add, \
    Activation, GlobalAveragePooling3D, multiply, Lambda
...
history = model.fit(train_dataset, epochs=50, validation_data=val_dataset, steps_per_epoch=len(X_train) // batch_size,
                    validation_steps=len(X_val) // batch_size, callbacks=[early_stopping])

# Save the trained model
model.save('3D_ResNet_CNN_with_attention_mechanism_smote.h5')
