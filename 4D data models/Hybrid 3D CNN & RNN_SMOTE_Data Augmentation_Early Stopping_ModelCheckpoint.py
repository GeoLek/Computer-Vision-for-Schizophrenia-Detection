...
from tensorflow.keras.layers import Input, Conv3D, MaxPooling3D, Flatten, Dense, Dropout, BatchNormalization, Activation, TimeDistributed, LSTM
...
from tensorflow.keras.layers import TimeDistributed, LSTM, GlobalAveragePooling3D
...
if physical_devices:
    try:
        ...
        print(f"{len(physical_devices)} Physical GPUs, {tf.config.experimental.list_logical_devices('GPU')} Logical GPUs.")
...
# Data Generator function to yield data in batches
...
X_resampled = X_resampled.reshape((-1, 65, 77, 49, X.shape[-1]))

# Define the simple 3D CNN architecture
...
def create_simplified_hybrid_3dcnn_rnn(input_shape, num_classes=2):
    ...
    x = TimeDistributed(Conv3D(16, kernel_size=3, padding='same', activation='relu'))(x)
    x = TimeDistributed(MaxPooling3D(pool_size=2))(x)
