...
# Define the simple 4D CNN model
def create_simple_4d_cnn(input_shape, num_classes=2):
...
input_shape = (65, 77, 49, X.shape[-1], 1)
...
for train_index, val_index in kfold.split(X_resampled):
    ...
    train_dataset = Dataset.from_generator(train_gen, output_types=(tf.float32, tf.float32),
                                           output_shapes=((batch_size, 65, 77, 49, X.shape[-1], 1), (batch_size, 2)))
    val_dataset = Dataset.from_generator(val_gen, output_types=(tf.float32, tf.float32),
                                         output_shapes=((batch_size, 65, 77, 49, X.shape[-1], 1), (batch_size, 2)))

    model = create_simple_4d_cnn(input_shape)
    model.summary()
