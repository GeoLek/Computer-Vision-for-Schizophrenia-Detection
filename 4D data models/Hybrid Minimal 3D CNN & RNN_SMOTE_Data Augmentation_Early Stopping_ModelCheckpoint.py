...
from tensorflow.keras.layers import Input, Conv3D, MaxPooling3D, Flatten, Dense, Dropout, TimeDistributed, LSTM, GlobalAveragePooling3D
...
def data_generator(data, labels, batch_size, target_shape, augment=False):
...
batch_size = 1
target_shape = (65, 77, 49, 100)  # Define target shape for padding/truncating with a time dimension
...
X_resampled = X_resampled.reshape((-1, 65, 77, 49, X.shape[-1] // (65 * 77 * 49)))
X_resampled = np.array([pad_or_truncate(x, target_shape) for x in X_resampled])
...
for train_index, val_index in kfold.split(X_resampled):
    ...
    train_gen = data_generator(X_train, y_train, batch_size, target_shape, augment=True)
    val_gen = data_generator(X_val, y_val, batch_size, target_shape)

    train_dataset = Dataset.from_generator(train_gen, output_types=(tf.float32, tf.float32),
                                           output_shapes=((batch_size, None, 65, 77, 49, 1), (batch_size, 2)))
    ...
    all_train_accuracies.append(
        np.pad(history.history['accuracy'], (0, max(0, 20 - len(history.history['accuracy']))), mode='constant',
               constant_values=np.nan))
    all_val_accuracies.append(
        np.pad(history.history['val_accuracy'], (0, max(0, 20 - len(history.history['val_accuracy']))), mode='constant',
               constant_values=np.nan))
    all_train_losses.append(
        np.pad(history.history['loss'], (0, max(0, 20 - len(history.history['loss']))), mode='constant',
               constant_values=np.nan))
    all_val_losses.append(
        np.pad(history.history['val_loss'], (0, max(0, 20 - len(history.history['val_loss']))), mode='constant',
               constant_values=np.nan))
