...
from tensorflow.keras.layers import Input, Conv3D, MaxPooling3D, Flatten, GlobalAveragePooling3D, Dense, Dropout, BatchNormalization, Activation
...
# Define the minimal 4D CNN model
def create_minimal_4d_cnn(input_shape, num_classes=2):
...
for train_index, val_index in kfold.split(X_resampled):
    ...
    model = create_minimal_4d_cnn(input_shape)
    model.summary()
    ...
    # Training the model
    history = model.fit(train_dataset, epochs=20, validation_data=val_dataset, steps_per_epoch=len(X_train) // batch_size,
