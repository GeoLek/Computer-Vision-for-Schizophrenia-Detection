...
from tensorflow.keras.layers import Conv3D, MaxPooling3D, Dense, GlobalAveragePooling3D, Input, TimeDistributed, Flatten
...
# Define a model with TimeDistributed Conv3D
def create_4d_cnn(input_shape, num_classes=2):
    inputs = Input(shape=input_shape, dtype=tf.float32, name='inputs')

    x = TimeDistributed(Conv3D(8, kernel_size=(3, 3, 3), padding='same', activation='relu'))(inputs)
    ...
    x = Flatten()(x)  # Flatten the time-distributed output
...
# Create the model
model = create_4d_cnn(input_shape)
model.summary()
